...
const FilterScreen = ({navigation}) => {
  ...
  return(
    ...
    <RadioButton value="gaussian" />
    <Text style={styles.textStyle}>Gaussian Filter</Text>
    </View>
    {value=="gaussian"?
    ...
    <Text style={styles.textStyle}>Mean Filter</Text>
    </View>
    {value=="mean"?
    <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', alignSelf: 'stretch', marginLeft: 10}}>
    <View style={{flexDirection: 'column', width: '100%'}}>
    ...
    <Text style={styles.textStyle}>Laplacian Filter</Text>
    </View>
    {value=="laplacian"?
    <View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', alignSelf: 'stretch', marginLeft: 10}}>
    <View style={{flexDirection: 'column', width: '100%'}}>
    ...
    {indexx==1?old_img="begin.jpg":old_img=pipeline[indexx - 2].imgName+".jpg"}
      ...
      if(image == '' || value == ''){
        ...
        if(value == 'gaussian'){
          var config = {
            method: 'get',
            url: 'https://f7gbmdtbzi.execute-api.us-east-2.amazonaws.com/v2/gaussian-filter?username='+user.email+'&old_img='+old_img+'&new_img='+image+'.jpg'+'&filter_size='+size1,
            headers: {
          ...
          var config = {
            method: 'get',
            url: 'https://f7gbmdtbzi.execute-api.us-east-2.amazonaws.com/v2/average-filter?username='+user.email+'&old_img='+old_img+'&new_img='+image+'.jpg'+'&filter_size='+size2,
            headers: {
              'Content-Type': 'application/json'
            }
          };
          ...
          var config = {
            method: 'get',
            url: 'https://f7gbmdtbzi.execute-api.us-east-2.amazonaws.com/v2/median-filter?username='+user.email+'&old_img='+old_img+'&new_img='+image+'.jpg'+'&filter_size='+size3,
            headers: {
              'Content-Type': 'application/json'
            }
          };
          ...
          var config = {
            method: 'get',
            url: 'https://f7gbmdtbzi.execute-api.us-east-2.amazonaws.com/v2/laplacian?username='+user.email+'&old_img='+old_img+'&new_img='+image+'.jpg'+'&filter_size='+size4,
            headers: {
              'Content-Type': 'application/json'
            }
          };
            ...
            axios(config).then((response)=> {
              axios.post('http://192.168.1.24:9000/api/PipeLine/', {
                ...
                "techniqueName": "Laplacian Filter",
                "technique": "laplacian",
                "satgeName":image,
                "imageURL": response.data.url,
                "position": indexx
